...
Architecture reference: `documentation/ARCHITECTURE.md` (ensure implementation matches the documented run layout, metrics/logging paths, and controller/trainer contracts).

## Feature suite specified in `documentation/TUI_FEATURES_PLAN.md` (copied)
...
# Dashboard:
# - press `e` to extend a run, name it, confirm start
# - press `r` (if chosen) to open gating replay for a seed
# - press `i` for System/Inference and verify server management panel
# - press `o` (if chosen) for Oracle screen
```
